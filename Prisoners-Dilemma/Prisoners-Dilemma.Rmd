...
2. Tit for Tat Variants:
- Tit for Tat
- Tit for Two Tats
- Tit for Tat with Forgiveness
- Tit for Tat with Randomization

3. Win-Stay/Lose-Switch Strategies:
- Pavlov 


4. Punishment-Based Strategies:
- Grim Trigger
- Bully
- Retaliatory Defector

5. Adaptive/Adjusting Strategies:
- Adaptive Defector
...
6. Gradient/Probability-Based Strategies:
...
<!-- ```{r} -->
<!-- #delete this chunk later, just for testing kable -->
<!-- fullresults_rank<-ranking_matrix_for_sequence_p(strategies, rounds = 1, rank = FALSE) -->
<!-- fullresults_total<-ranking_matrix_for_sequence_p(strategies, rounds = 1, rank = FALSE) -->

<!-- ``` -->


```{r}
fullresults_rank %>% format_table2()
```




```{r,fig.cap="Strategy Ranks Across Different Values of p\\label{pvalues}", fig.align='center'}

highlight<-c(    'Always Cooperate',
    'Always Defect',
    'Tit for Tat' ,'Grim/Trigger' )

fullresults_total %>% filter(  Strategy %in% highlight) %>% 
                           plot_total_utilities()
```
...
```{r}
fullresults_total %>% format_table2( title = "Strategy Values Across Different p Values")
```
